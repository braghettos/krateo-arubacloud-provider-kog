...
| # | Blocker | Scope | Owner |
|---|---|---|---|
| ~~P0-4~~ | **Read-only resources have no usable identifier.** `LoadBalancer` declares `identifiers: [name]`, but a read-only resource has no create body, so no identifier field is ever materialised in its CRD spec — which holds only `configurationRef` and `projectId`. There is no way to say *which* load balancer is meant: `findby` can never match, and `get` binds `{id}` from a `status.id` nothing populates. ~~Unusable by construction.~~ **Fixed in 0.22.1** ([#75](https://github.com/krateo-platformops/oasgen-provider/issues/75)) — a `name` selector is now generated and verified accepted. | `network/LoadBalancer` | this repo + upstream |
| P0-1 | **Token expires hourly.** ESO is now installed and wired (`eso/`): a `ClusterSecretStore` webhook generator mints tokens from a stored client credential, and an `ExternalSecret` refreshes `arubacloud-token` every 20 minutes against a ~1h lifetime. The plumbing is verified — the only remaining error is the deliberately-absent credential, stored once via `scripts/eso-bootstrap.sh`. **Closed** — the credential was stored 2026-09-01 06:25 and ESO was still minting working tokens ~29 hours later with no human involvement. | all tiers | this repo |
| P0-2 | **GA-core lifecycle evidence.** 4 GA + 1 beta of 9. Full lifecycles proven live: Subnet, KeyPair, Vpc, Project — the last two including out-of-band drift correction. Folder is beta: created and deleted cleanly, but drift was not exercised because neither of its mutable fields is safe to perturb. Remaining: SecurityGroup, SecurityRule, VpcPeering, VpcPeeringRoute. | GA core | this repo |
| ~~P0-5~~ | **Empty arrays in a CR spec are unenforceable.** RDC's `compareSlices` iterates only the CR's slice, so an empty one matches any remote value and the controller reports `Ready` while diverged. Emptying any list is invisible to drift detection; positional comparison makes reordering invisible too. ~~Found live.~~ **Fixed in 0.22.1** ([#76](https://github.com/krateo-platformops/oasgen-provider/issues/76)) — re-tested against the live API and corrected in ~3 min. | all tiers | upstream |
...
| ~~P0-6~~ | ~~**0.22.1 regression: deletes hang forever.**~~ **Fixed in 0.22.2** ([#99](https://github.com/krateo-platformops/oasgen-provider/pull/99)) and re-verified live: delete now completes in ~17s, the CR is released, and the resource returns 404. Original text: A 404 on native DELETE is treated as an error, so once the resource is actually gone every retry 404s and the finalizer never releases. Resources cannot be deleted through Kubernetes on an async-delete API. Filed as [#98](https://github.com/krateo-platformops/oasgen-provider/issues/98). **Blocks the storage and database waves.** | all tiers | upstream |
| P0-7 | **Upgrading does not regenerate existing CRDs.** A generator fix does not reach already-deployed resources; only deleting and re-applying the RestDefinition produces the new schema. Controller Deployments *are* updated, so the gap is specific to CRD schemas. Sharper still: deleting a RestDefinition regenerates its **resource** CRD but leaves the `<Kind>Configuration` CRD in place, so that one cannot be refreshed at all short of deleting it by hand — verified by rebuilding all 34, after which every resource CRD was new and all 34 Configuration CRDs kept their original timestamp. | all tiers | upstream |
